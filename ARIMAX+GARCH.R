...
RMSE(exp(ypred), exp(test_y[(length(test_y)-59):length(test_y)])) # optimal-(1,1,3),window=130: 1978092
...
for(i in 1:(n-window)){
  ...
  fit.garch = fGarch::garchFit(formula = ~garch(3,5), data=resid(fit.arimax),trace=F)
  garch_pred =  fGarch::predict(fit.garch)$meanForecast[1]
  arimax_pred = predict(fit.arimax,newxreg =armx_test_x[(end+1),] %>% t())$pred[1]
  pred = arimax_pred + garch_pred
  ypred = c(ypred, pred )
}
RMSE(exp(ypred), exp(test_y[(length(test_y)-59):length(test_y)])) # arima(1,1,3)+garch(3,5) : 1993206
